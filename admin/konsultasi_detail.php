...
if (isset($_POST["submit_tanggapan"])) {
  $insert = $con_pdo->prepare("INSERT INTO tanggapan (id_konsultasi, pemberi_tanggapan, isi_tanggapan) VALUES (:id_konsul, :bk, :messages)");
    $data_insert = array(
      "id_konsul" => $_POST["id_konsultasi"],
      "bk" => $_SESSION['admin']['nip'],
      "messages" => $_POST["tanggapan"]
   );
    $insert->execute($data_insert);
    header('Location: /desi/admin/konsultasi_detail.php?konsultasi='.$_GET['konsultasi']);
    exit();
}
?>
...
<div class="card mb-5">
  <ul class="list-group list-group-flush">
    ...
    <li class="list-group-item">
      <div class="d-flex <?php echo $data_chat[$value]['identifier'] == 'admin' ? "justify-content-end ms-5" : "me-5" ?>">
      <?php 
      if ($data_chat[$value]['identifier'] == 'admin') {
        ?>
        <div class="ms-5">
  ...
  <div class="card-footer">
    <form action="konsultasi_detail.php?konsultasi=<?php echo $_GET['konsultasi'] ?>" method="post">
      <div class="input-group">
        <input type="hidden" class="form-control" name="id_konsultasi" value="<?php echo $_GET["konsultasi"] ?>">
        <input type="text" class="form-control" name="tanggapan" placeholder="Type a message">
